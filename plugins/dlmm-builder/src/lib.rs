...
//! transactions **in-wasm** (waki) and emit a `solana-action:` URL.
//!
//! ## Structure
//!
//! - `build` — pure action building (host-testable): args parsing, mechanical
//!   validation, tx encoding, URL emission.
...
//! ## Security model
//!
//! The RPC endpoint, owner pubkey, relay base URL, and DLMM program id all
//! come from the plugin config section, which the host injects as `__config`
//! (caller-supplied values are stripped). The LLM can only pick *which*
//! position to act on and the *target range* — it cannot redirect the RPC,
//! change the signer, or tamper with the destination program. Mechanical
//! validation ("LLM proposes, plugin verifies") rejects anything that would
//! touch a position the operator doesn't own.

pub mod build;

#[cfg(target_family = "wasm")]
mod component {
    use super::build::{build_action, parse_key, ActionInput, Args};
    ...
    impl Tool for DlmmBuilder {
        ...
        fn description() -> String {
            "Build unsigned Meteora DLMM transactions. mode=claim: collect fees. \
             ...
             (ownership, claimable, range contains the active bin) and returns a \
             solana-action: URL for the stateless relay."
                .to_string()
        }
        ...
        fn parameters_schema() -> String {
            serde_json::json!({
                ...
                "properties": {
                    ...
                    "nonce_address": {
                        "type": "string",
                        "description": "Durable nonce account (recentBlockhash source)."
                    },
                    "new_low": {
                        "type": "integer",
                ...
                "required": ["mode", "position_id", "nonce_address"]
            })
            .to_string()
        }
        ...
        fn execute(args: String) -> Result<ToolResult, String> {
            ...
            let base_url = config
                .base_url
                .as_deref()
                .ok_or("__config.base_url missing — set the relay base URL")?;
            ...
            let nonce_address = args
                .nonce_address
                .as_deref()
                .ok_or("nonce_address required (durable nonce)")?;
            let nonce = parse_key(nonce_address, "nonce_address")?;

            // Fetch + decode position, pool, and nonce hash.
            ...
            let nonce_hash = rpc.get_nonce_hash_bytes(&nonce)?;
            ...
            let bin_arrays = if args.mode == "rebalance" {
